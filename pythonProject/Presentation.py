...
def open_window_session(id_patient,id_plan):
    ...
    while True:
        ...
        if event == "-START-":
            sg.Print("Start session by pulling the cable")
            cnt_success_reps = 0
            cnt_fail_sets = 0
            for i in range(len(df_of_rehab_plans)):
                if df_of_plans.loc[i,'Plan_id'] == df_of_rehab_plans.loc[i, 'Plan_id']:
                    set_time = df_of_rehab_plans['rest_between_sets[sec]'][id_plan]
                    rep_time = 4
                    magnet_on = b'\x01'
                    magnet_off = b'\x02'
                    magnet_weight = b'\x05'
                    force_limit = 2530
                    set_amount = df_of_rehab_plans["num_of_sets"][id_plan]
                    rep_amount = df_of_rehab_plans["num_of_reps"][id_plan]
                ...
                if cnt_sets < set_amount + 1:
                    if datapacket > 0 and datapacket < force_limit:
                        ...
                        if elapsed_time >= rep_time:
                            ...
                            print("this is a failed set no {}. please rest for the next set".format(cnt_fail_sets))
                            i = 0
                            while i < set_time :
                                time.sleep(1)
                                i = i + 1
                                time_delay = int(time.time() - start_time)
                                sg.Print('errored set, waiting to new set, {:02d}:{:02d}:{:02d}'.format(time_delay // 3600,
                                                                                                     (
                                                                                                             time_delay % 3600 // 60),
                                                                                                     time_delay % 60))
                            ...
                            cnt_sets = cnt_sets + 1
                            if(cnt_sets!= set_amount+1):
                                print("Starting set {} , please start pulling".format(cnt_sets))
                            cnt_reps = 1
                            window['Num of sets'].update(cnt_sets)
                            window['Num of reps'].update(cnt_reps)
                            data = magnet_on + magnet_weight
                    ...
                    if datapacket > force_limit:
                        ...
                        while i < rep_time:
                            ...
                            time_delay = int(time.time() - start_time)
                            sg.Print('rep finished {}.please release {:02d}:{:02d}:{:02d}'.format(cnt_reps, time_delay // 3600,
                                                                                         (time_delay % 3600 // 60),
                                                                                         time_delay % 60))
                        if cnt_reps < rep_amount + 1:
                            sg.Print("Resume pulling")
                            arduinoData.reset_input_buffer()
                            ##
                            cnt_reps = cnt_reps + 1
                            #print(cnt_reps)
                            ...
                            while i < set_time :
                                ...
                                i = i + 1
                                time_delay = int(time.time() - start_time)
                                sg.Print('set finished waiting:, {:02d}:{:02d}:{:02d}'.format(time_delay // 3600,
                                                                                           (time_delay % 3600 // 60),
                                                                                           time_delay % 60))
                        start_time = time.time()
                        cnt_success_reps += 1
                    ...
                    sg.Print('total reps achieved {} from {}'.format(cnt_success_reps, rep_amount * set_amount))
                    break
...
def first_window_patient():
    ...
    while True:
        ...
        #      print(int(values["Physiotherapist_id"]))
        if events1 == sg.WIN_CLOSED or events1 == 'Exit':
            break
        if events1 == 'Login':
            for i in range(len(df_of_patients)):
                if int(values1['Patient_id']) == df_of_patients.loc[i, 'Patient_id']:
                    ...
                    while True:
                        ...
                        if event == "Start_session":
                            window0.close()
                            open_window_session(values1['Patient_id'],plan_id1)
                            break
